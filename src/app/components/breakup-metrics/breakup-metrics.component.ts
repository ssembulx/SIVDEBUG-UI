 ...
 */
@Component({
  selector: 'cqi-breakup-metrics',
  templateUrl: './breakup-metrics.component.html',
  styleUrls: ['./breakup-metrics.component.css']
})
export class BreakupMetricsComponent implements OnInit {
  ...
  getDSI(duration?: string, groupDomain?: string, SKUIDcombination?: string, platform?: string, exposure?: String) {
    ...
    this.dataSvc.getBreakUpMetrics(data).subscribe(res => {
      if (res) {
        this.getDcrDetailsList = res.breakupMetricsDetails;
  ...
  renderBreakUpMetrics() {
    ...
    function createTraigeLineSeries() {
      ...
      lineSeries.dataFields.valueY = "averagedays_open";
