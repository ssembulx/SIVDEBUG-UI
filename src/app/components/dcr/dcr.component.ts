...
export class DcrComponent implements OnInit, OnDestroy {
  ...
  getDSI(duration?: string, groupDomain?: string, SKUIDcombination?: string, platform?: string, exposure?: String) {
    ...
    this.dataSvc.getDownloadData(downloadData).subscribe(res => {
      if (res) {
        this.dcrDownlaodData = res.dcrdownload;
      }
    });
    this.dataSvc.getDSIData(data).subscribe(res => {
      if (res) {
        this.getDcrDetailsList = res.dcrChart;
  ...
  exportTOExcel(data) {
    if (data == 0) {
      debugger;
      this.excelService.exportAsExcelFile(this.getDcrDetailsList, 'DCR data', this.dcrDownlaodData);
      //this.excelService.exportAsExcelFile(this.dcrDownlaodData, 'DCR raw data');
    } else if (data == 1) {
      this.excelService.exportAsExcelFile(this.groupDomainTraigeAccuracy, 'DCR Domain Comparison data');
    }
  }
  /* export to excel */
  ...
  renderComparisonDomain() {
    ...
    ToggleSeries.dataFields.categoryX = 'chartType'; //  d.valueX;
  ...
  renderChar() {
    ...
    // Create series
    function createSeries(field, name, percentage, Avgdays) {

      // Set up series
      var series = chart.series.push(new am4charts.ColumnSeries());
      series.dataFields.value = percentage;
      series.dataFields.customValue = Avgdays;
      series.name = name;
      //  series.className = Avgdays;
      series.dataFields.valueY = field;
      ...
      // Configure columns
      series.columns.template.width = am4core.percent(60);
      series.columns.template.tooltipText = "[#000 font-size: 14px]{name} [#000]:[/] [/][bold #000 font-size: 14px]{value}[/][bold #000]%[/] [/][#000]{additional}[/]\n [#000 font-size: 14px]Average Days[#000] : [bold #000 font-size: 14px]{customValue}";
    ...
    createSeries("cannot_reproduce", "Can Not Reproduce", "cannot_reproduce_percentage", "cannot_reproduce_Avgdays");
    createSeries("config_issue", "Config Issue", "config_issue_percentage", "config_issue_Avgdays");
    createSeries("behavior", "Expected Behavior", "behavior_percentage", "behavior_Avgdays");
    createSeries("feature_not_por", "Feature Not Por", "feature_not_por_percentage", "feature_not_por_Avgdays");
    createSeries("feature_por_not_ready", "Feature Por Not Ready", "feature_por_not_ready_percentage", "feature_por_not_ready_Avgdays");
    ...
    bulletOverAll.fill = am4core.color("#2196f3"); // tooltips grab fill from parent by default
    // bullet.tooltipText = "[#fff font-size: 15px]{name} in {categoryX}:\n[/][#fff font-size: 20px]{valueY}[/] [#fff]{additional}[/]"
    bulletOverAll.tooltipText = "[#000 font-size: 14px]{name} : [/][bold #000 font-size: 14px]{valueY} [/][bold #000]%[/][/] [#000]{additional}[/]";
    var circleOverAll = bulletOverAll.createChild(am4core.Circle);
    circleOverAll.radius = 3;
    circleOverAll.fill = am4core.color("#fff");
    circleOverAll.strokeWidth = 2;
