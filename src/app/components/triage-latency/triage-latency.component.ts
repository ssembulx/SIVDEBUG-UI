...
import { triageLatency } from "../../const/index";
 ...
 */
@Component({
  selector: 'cqi-triage-latency',
  templateUrl: './triage-latency.component.html',
  styleUrls: ['./triage-latency.component.css']
})
export class TriageLatencyComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.helper.GetUser().subscribe(user => {
      if (user != null) {
        // this.userInfo = user;
        this.dataSvc.getUserviewDetails(user.idsid, 'Triage latency').subscribe(res => {
          if (res) {
          }
        });
      }
    ...
    this.triageAccuracyData = triageLatency;
    this.spinner.show();
  ...
  getDSI(duration?: string, groupDomain?: string, SKUIDcombination?: string, platform?: string, exposure?: String) {
    ...
    this.dataSvc.getTriageLatency(data).subscribe(res => {
      if (res) {
        this.getDcrDetailsList = res.triageLatency;
    ...
    this.dataSvc.getTriageLatencyDownload(dat).subscribe(res => {
      if (res) {
        this.triageLatencyDownloadData = res.triagedoenload;
  ...
  renderChar() {
    ...
    valueAxis.min = 0;
    valueAxis.title.text = '[bold] Days';
    ...
    createSeries("oneDay", "New to Triage exit (<=1 day)", "onedaypercent");
    createSeries("onetotwoDay", "New to Triage exit (1-5 days)", "onetotwodayspercent");
    createSeries("twotothreeDays", "New to Triage exit (5-7 days)", "twotothreeDaysprecent");
    createSeries("greaterthenthree", "New to Triage exit (>7 days)", "greaterthenthreepercent");
    ...
    function createLineSeries() {
      ...
      lineSeriesDefects.name = "% sightings going through triage";
      lineSeriesDefects.dataFields.valueY = "sightingPercentage";
  ...
  exportTOExcel(data) {
    if (data == 1) {
      this.excelService.exportAsExcelFile(this.triageLatencyDownloadData, 'Triage Latency data');
    }
  }
  /* export to excel */
}
