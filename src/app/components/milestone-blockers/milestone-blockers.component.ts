...
import { milestoneBlockers } from "../../const/index";
 ...
 */
@Component({
  selector: 'cqi-milestone-blockers',
  templateUrl: './milestone-blockers.component.html',
  styleUrls: ['./milestone-blockers.component.css']
})
export class MilestoneBlockersComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.helper.GetUser().subscribe(user => {
      if (user != null) {
        //this.userInfo = user;
        this.dataSvc.getUserviewDetails(user.idsid, 'Milestone Blockers').subscribe(res => {
          if (res) {
          }
        });
      }
    ...
    this.triageAccuracyData = milestoneBlockers;
    this.spinner.show();
    this.chartView = false;
    this.noDataFound = false;
    //  this.getDSI("Year", "Group Domain", "Platform", "Exposure", "0");
    // this.getGroupDomainData();
    // this.getPlatformData();
  ...
  isDomainGroup = true;
  isPlatform = true;
  getDSI(duration?: string, groupDomain?: string, SKUIDcombination?: string, platform?: string, exposure?: String, NonIntelDefects?: any) {
    this.spinner.show();
    this.chartView = false;
    this.noDataFound = false;
    ...
    this.dataSvc.getMilestoneBlockers(data).subscribe(res => {
      if (res) {

        this.getDcrDetailsList = res.mileStoneBlockersDetail;
        this.dcrInformation = res.milestoneInformation;
  ...
  renderMilestoneBlockers() {
    ...
    //var tempArry = this.getDcrDetailsList;
    // var tempArry = Object.getOwnPropertyNames(this.getDcrDetailsList[0]);
    ...
    categoryAxis.title.text = "[bold] Milestone";
    ...
    valueAxis.min = 0;
    valueAxis.title.text = '[bold] Count of HSDs';
    ...
    /*  createSeries("lessthen30dayas", "<30 days");
     createSeries("between30to50days", "30-50 Days");
     createSeries("between50to60Days", "50-60 Days");
     createSeries("greaterthen60Dayas", ">60 days"); */
    var arrayList = [];
    chart.data.forEach((d, index) => {
    ...
    arrayList.forEach((d, index) => {
      ...
      if (d != "target") {
        ...
        eval('var series' + index + '= createSeries(d, d) ' + ';');
        //this["series" + index] = createSeries(d, d);
      }
    });
    //var series20 = createSeries("void", "Toggle All");
    /*   var series1 = createSeries("alderLakePlatforms", 'Alder Lake');
      var series2 = createSeries("amberLakePlatforms", 'Amber Lake');
      var series3 = createSeries("basinFallsPlatforms", 'Basin Falls');
      var series4 = createSeries("broadwellPlatforms", 'Broadwell');
      var series5 = createSeries("broxtonPlatforms", 'Broxton');
      var series6 = createSeries("cannonLakePlatforms", 'Cannon Lake');
      var series7 = createSeries("coffeeLakePlatforms", 'Coffee Lake');
      var series8 = createSeries("cometLakePlatforms", 'Comet Lake');
      var series9 = createSeries("fishhawkFallsMSWSPlatforms", 'Fishhawk Falls MSWS');
      var series10 = createSeries("fishhawkFallsPlatforms", 'Fishhawk Falls');
      var series11 = createSeries("glacierFallsPlatforms", 'Glacier Falls');
      var series12 = createSeries("iceLakePlatforms", 'Ice Lake');
      var series13 = createSeries("jasperLakePlatforms", 'Jasper Lake');
      var series14 = createSeries("kabyLakePlatforms", 'Kaby Lake');
      var series15 = createSeries("lakefieldPlatforms", 'Lakefield');
      var series16 = createSeries("rocketLakePlatforms", 'Rocket Lake');
      var series17 = createSeries("skylakePlatforms", 'Sky Lake');
      var series18 = createSeries("tigerLakePlatforms", 'Tiger Lake');
      var series19 = createSeries("whiskeyLakePlatforms", 'Whiskey Lake');
      var series20 = createSeries("void", "Toggle All"); */
  ...
  exportTOExcel() {
    ...
    this.chartView = false;
    this.noDataFound = false;
    let groupDomain, platform, exposure
    if (this.selectedGroupDomain == "Group Domain") {
      groupDomain = "";
    }
    else {
      groupDomain = this.selectedGroupDomain;
    }
    ...
    this.dataSvc.getMilestoneBlockersDownloadData(data).subscribe(res => {
      if (res) {
        this.noDataFound = false;
        this.spinner.hide();
        this.chartView = true;
        // this.getDcrDetailsList = res.mileStoneBlockersDetail;
        this.excelService.exportAsExcelFile(res.milestonedownloaddata, 'Over all data');
      }
    });
  }
  /* export to excel */
