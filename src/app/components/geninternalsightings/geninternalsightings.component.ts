...
import { genInternal } from "../../const/gen-internal";
...
@Component({
  selector: 'cqi-geninternalsightings',
  templateUrl: './geninternalsightings.component.html',
  styleUrls: ['./geninternalsightings.component.css']
})
export class GeninternalsightingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.triageAccuracyData = genInternal;
    this.insideChart();
    this.dashboardChart();
    ...
    this.getGenInternalSightings("Year", "Group Domain", "Platform");
  ...
  dcrInformation;
  cumulativeChartData;
  getGenInternalSightings(duration?: string, groupDomain?: string, platform?: string) {
    this.spinner.show();
    this.chartView = false;
    this.noDataFound = false;
    let data;
    ...
    this.dataSvc.getGenInternalSightings(data).subscribe(res => {
      if (res) {
        this.cumulativeChartData = res;
        this.dcrInformation = res.genOverGenInsightConditionFixDetails;
        this.cumulativeChartData.genoverColourCode.forEach(d => {
        ...
        if (this.cumulativeChartData.data == null) {
          this.spinner.hide();
          this.chartView = false;
          ...
          this.spinner.hide();
          this.chartView = true;
          this.renderGenInternalSightings();
        }
      }
    });
  }
  ...
  async renderGenInternalSightings() {
    ...
    chart.data = this.cumulativeChartData.data;
    let titleHeader;
  ...
  onPlatformValChange(value) {
    ...
    this.chartView = false;
    this.selected = value;
    this.getGenInternalSightings(this.duration, this.selectedGroupDomain, this.selected);
  }
