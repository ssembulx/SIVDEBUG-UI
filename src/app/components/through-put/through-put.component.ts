 ...
 */
@Component({
  selector: 'cqi-through-put',
  templateUrl: './through-put.component.html',
  styleUrls: ['./through-put.component.css']
})
export class ThroughPutComponent implements OnInit, OnDestroy {
  ...
  getDSI(duration?: string, groupDomain?: string, SKUIDcombination?: string, platform?: string, exposure?: String) {
    ...
    this.dataSvc.getSIVBugThroughPut(data).subscribe(res => {
      if (res) {
        this.getDcrDetailsList = res.siVthroughput;
  ...
  renderChar() {
    ...
    createSeries('sigthingsSubmitted', 'Submitted Sightings');
    ...
    function createTraigeLineSeries() {
      ...
      yAxis.title.text = '[bold] Average ageing for backlogs';
      yAxis.renderer.labels.template.fill = am4core.color("#000000");
      yAxis.renderer.labels.template.fontSize = 12;
      ...
      lineSeries.name = "Average ageing for backlogs";
